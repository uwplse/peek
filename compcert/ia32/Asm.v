...
Lemma hfreg_eq: forall (x y: hfreg), {x=y} + {x<>y}.
Proof. decide equality. Defined.
...
Definition split_big_freg (b : bfreg) : (freg * hfreg) :=
  match b with
    | xmm0 => (XMM0,XMM0H)
    | xmm1 => (XMM1,XMM1H)
    | xmm2 => (XMM2,XMM2H)
...
Inductive preg: Type :=
  ...
  | FRH : hfreg -> preg                 (**r high 64 bits of XMM reg *)
...
Inductive instruction: Type :=
  ...
  | Pflds_m (a: addrmode)               (**r [fld] single precision *)
  | Pfstps_m (a: addrmode)              (**r [fstp] single precision *)
  | Pxchg_rr (r1: ireg) (r2: ireg)      (**r register-register exchange *)
  | Pmovups_rm (rd: bfreg) (a: addrmode)(**r 128 bit load *)
  | Pmovups_mr (a: addrmode) (rs: bfreg)(**r 128 bit store *)
  ...
  | Paddpd_ff (rd: bfreg) (r1: bfreg)
  | Pmulpd_ff (rd: bfreg) (r1: bfreg)
...
Proof. decide equality. apply ireg_eq. apply freg_eq.
       apply hfreg_eq. decide equality.
Defined.
...
(* compute an offset of an addrmode *)
Definition addr_add (a : addrmode) (z : Z) : addrmode :=
  match a with
    | Addrmode base ofs (inl i) =>
      Addrmode base ofs (inl (Int.add (Int.repr z) i))
    | Addrmode base ofs (inr (id,i)) =>
      Addrmode base ofs (inr (id,((Int.add (Int.repr z) i))))
  end.

Definition exec_big_load (chunk: memory_chunk) (m: mem)
                         (a: addrmode) (rs: regset) (rd: bfreg) :=
  let (lrd,hrd) := split_big_freg rd in
  match Mem.loadv chunk m (eval_addrmode a rs) with
    | Some v =>
      match Mem.loadv chunk m (eval_addrmode (addr_add a (size_chunk chunk)) rs) with
        | Some v' => Next (nextinstr_nf ((rs#lrd <- v)#hrd <- v')) m
        | None => Stuck
      end
    | None => Stuck
  end.

Definition exec_big_store (chunk: memory_chunk) (m: mem)
                          (a: addrmode) (rs: regset) (r1: bfreg)
                          (destroyed: list preg) :=
  let (lr1,hr1) := split_big_freg r1 in
  match Mem.storev chunk m (eval_addrmode a rs) (rs#lr1) with
    | Some m' =>
      match Mem.storev chunk m' (eval_addrmode (addr_add a (size_chunk chunk)) rs) (rs#hr1) with
...
Definition exec_instr (f: function) (i: instruction) (rs: regset) (m: mem) : outcome :=
  ...
  | Pmov_mi a n =>
    match Mem.storev Mint32 m (eval_addrmode a rs) (Vint n) with
      | Some m' => Next (nextinstr_nf rs) m'
      | None => Stuck
    end
  | Pmovsd_ff rd r1 =>
  ...
  | Pmovups_rm brd a =>
      exec_big_load Mfloat64 m a rs brd
  | Pmovups_mr a br1 =>
      exec_big_store Mfloat64 m a rs br1 nil
  (** Moves with conversion *)
  | Pmovb_mr a r1 =>
  ...
  | Pinc rd =>
      Next (nextinstr_nf (rs#rd <- (Val.add rs#rd Vone))) m
  | Pdec rd =>
      Next (nextinstr_nf (rs#rd <- (Val.sub rs#rd Vone))) m
  | Padd_rr rd r1 =>
      Next (nextinstr_nf (rs#rd <- (Val.add rs#rd rs#r1))) m
  | Psub_rr rd r1 =>
  ...
  | Paddpd_ff brd br1 =>
    ...
    let nrd' := Val.addf rs#rd' rs#r1' in
    Next (nextinstr ((rs#rd <- nrd)#rd' <- nrd')) m
  | Pmulpd_ff brd br1 =>
    let (rd,rd') := split_big_freg brd in
    let (r1,r1') := split_big_freg br1 in
