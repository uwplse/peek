...
Definition transf_rtl_program (f: RTL.program) : res Asm.program :=
  ...
  @@@ time "AsmBits generation" AsmBitsGen.transf_program  
  @@@ partial_if Compopts.optim_peeps (time "Peephole optimization" Peepholes.transf_program).
...
Theorem transf_rtl_program_correct:
  ...
  forward_simulation (RTL.semantics p) (AsmBits.semantics_bits tp)
  * backward_simulation (RTL.semantics p) (AsmBits.semantics_bits tp).
Proof.
  intros.
  assert (F: forward_simulation (RTL.semantics p) (AsmBits.semantics_bits tp)).
  ...
  destruct (Asmgen.transf_program p8) as [p9|] eqn:?; simpl in H; try discriminate.
  destruct (AsmBitsGen.transf_program p9) as [p10|] eqn:?; simpl in H; try discriminate.
  ...
  apply compose_forward_simulation with (Mach.semantics Asmgenproof0.return_address_offset p8).
    ...
    auto.
  apply compose_forward_simulation with (Asm.semantics p9).
  apply Asmgenproof.transf_program_correct; eauto.
  apply compose_forward_simulation with (AsmBits.semantics_bits p10).
  apply AsmBitsGen.transf_inv in Heqr7.
  repeat break_and. subst.
  apply Asm2Bits.as_bits_fsim; auto.

  
  admit. (* Core assumption about mapping infinite memory to finite memory *)
  
  eapply partial_if_simulation; eauto.
  apply Peepholes.transf_program_correct.    
    
  split. auto. 
  apply forward_to_backward_simulation. auto. 
  apply RTL.semantics_receptive.
  apply Asm2Bits.semantics_determinate.
Qed.

Theorem transf_cminor_program_correct:
  forall p tp,
  transf_cminor_program p = OK tp ->
  forward_simulation (Cminor.semantics p) (AsmBits.semantics_bits tp)
  * backward_simulation (Cminor.semantics p) (AsmBits.semantics_bits tp).
Proof.
  intros.
  assert (F: forward_simulation (Cminor.semantics p) (AsmBits.semantics_bits tp)).
...
Theorem transf_clight_program_correct:
  ...
  forward_simulation (Clight.semantics1 p) (AsmBits.semantics_bits tp)
  * backward_simulation (Clight.semantics1 p) (AsmBits.semantics_bits tp).
Proof.
  intros. 
  assert (F: forward_simulation (Clight.semantics1 p) (AsmBits.semantics_bits tp)).
...
Theorem transf_cstrategy_program_correct:
  ...
  forward_simulation (Cstrategy.semantics p) (AsmBits.semantics_bits tp)
  * backward_simulation (atomic (Cstrategy.semantics p)) (AsmBits.semantics_bits tp).
Proof.
  intros.
  assert (F: forward_simulation (Cstrategy.semantics p) (AsmBits.semantics_bits tp)).
  ...
  eapply Asm2Bits.semantics_determinate.
...
Theorem transf_c_program_correct:
  forall p tp,
  transf_c_program p = OK tp ->
  backward_simulation (Csem.semantics p) (AsmBits.semantics_bits tp).
Proof.
  intros. 
  apply compose_backward_simulation with (atomic (Cstrategy.semantics p)).
  eapply sd_traces; eapply Asm2Bits.semantics_determinate.
