...
Lemma alloc_access :
  forall b',
    Plt b' b ->
    forall x k,
      (mem_access m1) !! b' x k = (mem_access m2) !! b' x k.
Proof.
  intros. unfold alloc in *.
  inversion ALLOC. simpl.
  eapply Plt_ne in H.
  rewrite PMap.gso by congruence. reflexivity.
Qed.

Lemma alloc_access_new :
  forall ofs,
    lo <= ofs < hi ->
    (Mem.mem_access m2) !! b ofs Cur = Some Freeable.
Proof.
  unfold alloc in *.
  inversion ALLOC.
  intros. simpl.
  ...
  destruct (zle lo ofs) eqn:?;
           destruct (zlt ofs hi) eqn:?;
           try omega; congruence.
Qed.

Lemma alloc_access_result :
  mem_access m2 = PMap.set (nextblock m1)
...
Theorem mem_contents_alloc :
  mem_contents m2 = PMap.set b (ZMap.init Undef) (mem_contents m1).
Proof.
  injection ALLOC; intros. rewrite <- H0. simpl.
  rewrite alloc_result. reflexivity.
Qed.
